...
namespace nsK2Engine {
    ...
    )
    {
        ...
        for (int areaNo = 0; areaNo < NUM_SHADOW_MAP; areaNo++)
        {
            ...
            // このカスケードの深度範囲にぴったり合う投影行列を作る。
            // 全カスケードで同じ広い深度範囲を使うと深度精度が足りなくなり、
            // 近距離用(高解像度)のシャドウマップでも影のバイアスがワールド座標で
            // 大きくなりすぎて、影が足元から浮いて見える原因になっていた。
            // 視錐台スライスの外側にいる遮蔽物(画面外の高いオブジェクトなど)まで
            // 影を落とせるように、近平面側には奥行きの半分だけ余裕を持たせる。
            float depthRange = max(1.0f, farZ - nearZ);
            float cascadeNear = max(1.0f, nearZ - depthRange * 0.5f);
            float cascadeFar = max(cascadeNear + 1.0f, farZ);
            Matrix projMatrix;
